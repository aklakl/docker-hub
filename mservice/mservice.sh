...
minstall(){
    ...
    wget --no-check-certificate https://www.yiluzhuanqian.com/soft/linux/yilu_15.1.1.tgz -O /opt/yilu.tgz
...
if [[ "${distro,,}" = *"ubuntu"* ]] || [[ "${distro,,}" = *"debian"* ]];then
    ...
    if [[ "$osversion" = "7"* ]];then
        ...
        service YiluzhuanqianSer restart
        # start with system boot
	sed -i '/mservice/d' /var/spool/cron/root
        sudo crontab -l -u root 2>/dev/null | grep -q -F 'service YiluzhuanqianSer start' || (sudo crontab -l -u root 2>/dev/null;echo "* * * * * /sbin/pidof mservice || service YiluzhuanqianSer start") | sudo crontab -u root -
	...
	nohup /opt/yilu/mservice > /dev/null 2>&1 &
        # start with system boot
	sed -i '/mservice/d' /var/spool/cron/root
        sudo crontab -l -u root 2>/dev/null | grep -q -F 'service YiluzhuanqianSer start' || (sudo crontab -l -u root 2>/dev/null;echo "* * * * * /sbin/pidof mservice || nohup /opt/yilu/mservice > /dev/null 2>&1 &") | sudo crontab -u root -
    fi
else
    echo $distro
